...
def plot_per_view_coverage(occlusion_rows: list[dict], fmt: str, dpi: int):
    ...
    ax.scatter(full[:, 0], full[:, 1], full[:, 2],
               c=cmap[labels], s=0.3, depthshade=True)
    ax.set_title(
...
def plot_benchmark_comparison(occlusion_rows: list[dict], summary_rows: list[dict], fmt: str, dpi: int):
    ...
    bp = ax.boxplot(box_scores, positions=box_positions, widths=box_width,
    ...
    for i, obj in enumerate(plot_objects):
        ...
        ax.plot(center, -0.12, 'o', color=color, markersize=5, clip_on=False,
...
def plot_synthetic_setup(fmt: str, dpi: int):
    ...
    def _draw_scene(ax, show_labels=False):
        ...
        if len(all_visible) > 0:
            ax.scatter(all_visible[:, 0] * 100, all_visible[:, 1] * 100, all_visible[:, 2] * 100,
                       c=POINT_COLOR, s=2, alpha=0.5,
        ...
        _draw_frustum(ax, wrist_frame, CAM_WRIST, "Wrist camera" if show_labels else None)
...
def _load_stage_means() -> dict:
    ...
    Returns dict with keys: twist_mean, twist_p95, seg_cpu_mean, seg_cpu_p95,
    seg_cuda_mean, seg_cuda_p95, preshape_mean, preshape_p95.
    """
    import numpy as np
    ...
    if ok_rows:
        twist_vals = [float(r["twist_propagation_ms"]) for r in ok_rows
                      if not _is_nan(r.get("twist_propagation_ms"))]
        preshape_vals = [float(r["preshaping_ms"]) for r in ok_rows
...
def plot_per_stage_waterfall(fmt: str = "png", dpi: int = 300):
    """Waterfall chart: Twist Propagation + Segmentation + Grasp Preshaping.

    Shows two grouped waterfall bars (CPU pipeline and CUDA pipeline) with
    per-stage breakdowns and a total bar for each.  Includes a box-strip
    inset showing the per-trial distribution of segmentation latency for
    both backends.
    """
    import matplotlib
    matplotlib.use("Agg")
    ...
    # Stage definitions: (label, color, mean_key)
    stages = [
        ("Twist\nPropagation", "#2ecc71", "twist_mean"),
        ("Segmentation",      "#e67e22", None),      # filled per-backend
        ("Grasp\nPreshaping", "#e74c3c", "preshape_mean"),
    ]

    seg_color_cpu = "#d4a574"
    seg_color_cuda = "#f5b041"

    # --- Layout: main waterfall (left 65%) + seg boxplot (right 35%) ---
    fig, (ax_main, ax_box) = plt.subplots(
        1, 2, figsize=(12, 5.5),
        gridspec_kw={"width_ratios": [1.8, 1], "wspace": 0.35})

    # ═══════════════════════════════════════════════════════════════════
    # Left panel: stacked waterfall bars (CPU and CUDA side by side)
    # ═══════════════════════════════════════════════════════════════════
    bar_width = 0.35
    x_positions = np.array([0, 1, 2, 3])  # twist, seg, preshape, total
    x_labels = ["Twist\nPropagation", "Segmentation", "Grasp\nPreshaping", "TOTAL"]

    # CPU stacked bar
    cpu_vals = [sm["twist_mean"], sm["seg_cpu_mean"], sm["preshape_mean"]]
    cpu_total = sum(cpu_vals)
    cpu_colors = [stages[0][1], seg_color_cpu, stages[2][1]]

    # CUDA stacked bar
    cuda_vals = [sm["twist_mean"], sm["seg_cuda_mean"], sm["preshape_mean"]]
    cuda_total = sum(cuda_vals)
    cuda_colors = [stages[0][1], seg_color_cuda, stages[2][1]]

    # Draw stacked segments
    for i, (cv, cudav, col_cpu, col_cuda) in enumerate(
            zip(cpu_vals, cuda_vals, cpu_colors, cuda_colors)):
        # CPU bar (left)
        bottom_cpu = sum(cpu_vals[:i])
        ax_main.bar(x_positions[i] - bar_width/2, cv, bar_width,
                    bottom=bottom_cpu, color=col_cpu, edgecolor="white",
                    linewidth=0.5)
        if cv > 8:
            ax_main.text(x_positions[i] - bar_width/2, bottom_cpu + cv/2,
                         f"{cv:.0f}", ha="center", va="center",
                         fontsize=8, color="white", fontweight="bold")

        # CUDA bar (right)
        bottom_cuda = sum(cuda_vals[:i])
        ax_main.bar(x_positions[i] + bar_width/2, cudav, bar_width,
                    bottom=bottom_cuda, color=col_cuda, edgecolor="white",
                    linewidth=0.5)
        if cudav > 8:
            ax_main.text(x_positions[i] + bar_width/2, bottom_cuda + cudav/2,
                         f"{cudav:.0f}", ha="center", va="center",
                         fontsize=8, color="white", fontweight="bold")

    # Total bars
    ax_main.bar(x_positions[3] - bar_width/2, cpu_total, bar_width,
                color="#7f8c8d", edgecolor="white", linewidth=0.5)
    ax_main.text(x_positions[3] - bar_width/2, cpu_total / 2,
                 f"{cpu_total:.0f}", ha="center", va="center",
                 fontsize=9, color="white", fontweight="bold")

    ax_main.bar(x_positions[3] + bar_width/2, cuda_total, bar_width,
                color="#2c3e50", edgecolor="white", linewidth=0.5)
    ax_main.text(x_positions[3] + bar_width/2, cuda_total / 2,
                 f"{cuda_total:.0f}", ha="center", va="center",
                 fontsize=9, color="white", fontweight="bold")

    # Threshold lines
    ax_main.axhline(y=400, color=COLORS["negative"], linestyle="--",
                    linewidth=1.5, label="MAR (400 ms)")
    ax_main.axhline(y=100, color=COLORS["warning"], linestyle=":",
                    linewidth=1.0, label="IDE (100 ms)")

    ax_main.set_xticks(x_positions)
    ax_main.set_xticklabels(x_labels, fontsize=9)
    ax_main.set_ylabel("Cumulative Latency (ms)", fontsize=10)
    ax_main.set_title("Pipeline Stage Latency Waterfall", fontsize=11,
                      fontweight="bold")
    ax_main.legend(loc="upper left", fontsize=8)

    # Backend legend
    cpu_patch = mpatches.Patch(color="#7f8c8d", label="CPU Pipeline")
    cuda_patch = mpatches.Patch(color="#2c3e50", label="CUDA Pipeline")
    ax_main.legend(
        handles=[cpu_patch, cuda_patch,
                 plt.Line2D([0], [0], color=COLORS["negative"],
                            linestyle="--", linewidth=1.5, label="MAR (400 ms)"),
                 plt.Line2D([0], [0], color=COLORS["warning"],
                            linestyle=":", linewidth=1.0, label="IDE (100 ms)")],
        loc="upper left", fontsize=7.5, framealpha=0.9)

    # ═══════════════════════════════════════════════════════════════════
    # Right panel: segmentation latency distribution (box + strip)
    # ═══════════════════════════════════════════════════════════════════
    cpu_trials_path = os.path.join(RESULTS_DIR, "segmentation_cpu_trials.csv")
    cuda_trials_path = os.path.join(RESULTS_DIR, "segmentation_cuda_trials.csv")

    cpu_trials_rows = _load_csv(cpu_trials_path)
    cuda_trials_rows = _load_csv(cuda_trials_path)

    cpu_latencies = [float(r["latency_ms"]) for r in cpu_trials_rows
                     if r.get("status") == "ok"]
    cuda_latencies = [float(r["latency_ms"]) for r in cuda_trials_rows
                      if r.get("status") == "ok"]

    if cpu_latencies and cuda_latencies:
        data = [cpu_latencies, cuda_latencies]
        box_labels = ["CPU", "CUDA"]

        # Box plot
        bp = ax_box.boxplot(data, labels=box_labels, patch_artist=True,
                            widths=0.5, showfliers=False,
                            medianprops={"color": "white", "linewidth": 2},
                            whiskerprops={"color": COLORS["text"], "linewidth": 1},
                            capprops={"color": COLORS["text"], "linewidth": 1})
        bp["boxes"][0].set_facecolor(seg_color_cpu)
        bp["boxes"][0].set_edgecolor(COLORS["text"])
        bp["boxes"][1].set_facecolor(seg_color_cuda)
        bp["boxes"][1].set_edgecolor(COLORS["text"])

        # Strip plot (jittered points)
        for i, (latencies, color) in enumerate(
                zip(data, [seg_color_cpu, seg_color_cuda])):
            jitter = np.random.default_rng(42).uniform(-0.12, 0.12, len(latencies))
            ax_box.scatter(np.full(len(latencies), i + 1) + jitter,
                           latencies, alpha=0.3, s=8, color=color,
                           edgecolors="none", zorder=2)

        # Annotate means
        for i, latencies in enumerate(data):
            mean_val = np.mean(latencies)
            ax_box.scatter([i + 1], [mean_val], marker="D", s=40,
                           color="white", edgecolors=COLORS["text"],
                           zorder=3, linewidths=1)
            ax_box.annotate(f"{mean_val:.0f} ms",
                            xy=(i + 1, mean_val),
                            xytext=(i + 1.35, mean_val),
        ...
        ax_box.set_ylabel("Segmentation Latency (ms)", fontsize=10)
        ax_box.set_title("Segmentation Backend\nComparison (100 trials each)",
                         fontsize=10, fontweight="bold")
    else:
        ax_box.text(0.5, 0.5, "No segmentation\nbenchmark data",
...
def plot_object_gallery(fmt: str, dpi: int):
    ...
    if sq_found_count > 0:
        ...
        print(f"  [SQ] No SQ debug dumps matched — showing point clouds only "
...
def plot_score_vs_samples(fmt: str, dpi: int):
    ...
    if not os.path.isfile(sweep_path):
        print("  Skipping fig10_score_vs_samples: no sweep data. "
    ...
    ax.errorbar(sample_counts, sv_means, yerr=sv_stds,
                  fmt='o-', color=COLORS["single_view"], label="Single-view",
                  capsize=3, markersize=5, linewidth=2)
    ax.errorbar(sample_counts, mv_means, yerr=mv_stds,
    ...
    if os.path.isfile(baseline_path):
        ...
        ax.axhline(bl_mean, color=COLORS["positive"], linestyle='--', linewidth=1.5,
...
def plot_latency_score_variability(fmt: str, dpi: int):
    ...
    fig, (ax_lat, ax_var) = plt.subplots(
        1, 2, figsize=(12, 5), facecolor=COLORS["bg"])
    fig.set_facecolor(COLORS["bg"])
    ...
    ax_lat.errorbar(sample_counts, lat_means, yerr=lat_stds,
                    fmt='o-', color=COLORS["warning"], capsize=3,
                    markersize=5, linewidth=2, label="Mean +/- 1 SD")
    ...
    for n in sample_counts:
        ...
        ax_lat.scatter(10 ** x, lats, alpha=0.15, s=6,
                       color=COLORS["warning"], edgecolors="none", zorder=1)

    ax_lat.axhline(100, color=COLORS["negative"], linestyle='--',
                   linewidth=1.5, label="IDE limit (100 ms)")
    ax_lat.axhline(400, color=COLORS["negative"], linestyle=':',
                   linewidth=1.5, label="MAR limit (400 ms)")
    ax_lat.set_xlabel("Prediction Samples", color=COLORS["text"])
    ax_lat.set_ylabel("Pipeline Latency (ms)", color=COLORS["text"])
    ax_lat.set_title("Latency vs Sample Count", color=COLORS["text"],
                     fontweight="bold")
    ...
    ax_var.errorbar(sample_counts, score_medians,
                    yerr=[score_iqr_lo, score_iqr_hi],
                    fmt='s-', color=COLORS["primary"], capsize=3,
                    markersize=5, linewidth=2, label="Median +/- IQR")
    ...
    for n in sample_counts:
        ...
        ax_var.scatter(10 ** x, scores, alpha=0.15, s=6,
                       color=COLORS["primary"], edgecolors="none", zorder=1)

    ax_var.set_xlabel("Prediction Samples", color=COLORS["text"])
    ax_var.set_ylabel("Combined Score", color=COLORS["text"])
    ax_var.set_title("Score Variability vs Sample Count", color=COLORS["text"],
                     fontweight="bold")
...
def plot_accuracy_gain(fmt: str, dpi: int):
    ...
    gains = [float(r["delta_grasp_accuracy_pct"]) for r in rows
    ...
    fig, ax = plt.subplots(1, 1, figsize=(4, 5.5), facecolor=COLORS["bg"])
    fig.set_facecolor(COLORS["bg"])

    # Box plot
    bp = ax.boxplot([gains], labels=["Accuracy Gain"], patch_artist=True,
    ...
    # Zero reference line
    ax.axhline(0, color=COLORS["negative"], linestyle='--', linewidth=1.2,
               label="No gain")

    # Annotate median and mean
    median_val = np.median(gains)
    mean_val = np.mean(gains)
    ax.scatter([1], [mean_val], marker='D', s=50, color='white',
               edgecolors=COLORS["text"], zorder=3, linewidths=1,
               label=f"Mean ({mean_val:+.1f}%)")
    ax.annotate(f"Median: {median_val:+.1f}%",
                xy=(1, median_val), xytext=(1.35, median_val),
                fontsize=9, color=COLORS["text"], fontweight="bold",
                arrowprops=dict(arrowstyle="-", color=COLORS["grid"],
                                linewidth=0.5))

    # Jittered strip of individual object gains
    rng = np.random.default_rng(42)
    jitter = rng.uniform(-0.12, 0.12, len(gains))
    ax.scatter(np.ones(len(gains)) + jitter, gains, alpha=0.5, s=20,
               color=COLORS["single_view"], edgecolors="none", zorder=2)

    ax.set_ylabel("Accuracy Gain (percentage points)", color=COLORS["text"],
                  fontsize=10)
    ax.set_title("Multi-view Accuracy Gain\n(per object)", color=COLORS["text"],
...
def plot_tier_b_latency(fmt: str, dpi: int):
    ...
    svc_rows = [r for r in rows if r.get("method") == "service" and r.get("status") == "ok"]
    ...
    if emg_rows:
        ...
        for field, label, color in stage_fields:
            values = [float(r[field]) for r in emg_rows
        ...
        for label, mean_val, color in stage_data:
            ax.bar(label, mean_val, bottom=cumulative, color=color, edgecolor="white", linewidth=0.5)
            if mean_val > 0:
        ...
        ax.axhline(y=400, color=COLORS["negative"], linestyle="--", linewidth=1.5,
                   label="MAR (400 ms)")
        ax.axhline(y=100, color=COLORS.get("warning", "#f39c12"), linestyle=":", linewidth=1.0,
                   label="IDE (100 ms)")
    ...
    if emg_rows:
        vals = [float(r["total_latency_ms"]) for r in emg_rows
    ...
    if svc_rows:
        vals = [float(r["total_latency_ms"]) for r in svc_rows
                if not _is_nan(r.get("total_latency_ms"))]
        if vals:
...
def main():
    ...
    plot_benchmark_comparison(occlusion_rows, summary_rows, args.format, args.dpi)  # Fig 4
