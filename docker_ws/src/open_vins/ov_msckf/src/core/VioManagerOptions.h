...
struct VioManagerOptions {
  ...
  static std::vector<int> parse_marker_ids(const std::string &ids_string) {
    ...
    return ids.empty() ? std::vector<int>{0} : ids;
  }
  ...
  void print_and_load_estimator(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    ...
    if (parser != nullptr) {
      ...
      parser->parse_config("marker_fixed_ids", marker_fixed_ids, false);
      marker_pose_options.fixed_marker_ids = parse_marker_ids(marker_fixed_ids);
      parser->parse_config("marker_time_tolerance_s", marker_pose_options.time_tolerance_s, false);
      parser->parse_config("marker_chi2_gate", marker_pose_options.chi2_gate, false);
      parser->parse_config("marker_noise_multiplier", marker_pose_options.noise_multiplier, false);
      parser->parse_config("marker_max_update_translation_m", marker_pose_options.max_update_translation_m, false);
      parser->parse_config("marker_max_update_rotation_deg", marker_pose_options.max_update_rotation_deg, false);
      parser->parse_config("marker_reset_translation_m", marker_pose_options.reset_translation_m, false);
      parser->parse_config("marker_reset_rotation_deg", marker_pose_options.reset_rotation_deg, false);
      parser->parse_config("marker_reset_min_samples", marker_pose_options.reset_min_samples, false);
      parser->parse_config("marker_reset_window_s", marker_pose_options.reset_window_s, false);
      parser->parse_config("marker_reset_min_sample_dt_s", marker_pose_options.reset_min_sample_dt_s, false);
      parser->parse_config("marker_reset_max_velocity_mps", marker_pose_options.reset_max_velocity_mps, false);
    ...
    for (size_t i = 0; i < marker_pose_options.fixed_marker_ids.size(); i++) {
      marker_ids_ss << marker_pose_options.fixed_marker_ids.at(i);
      if (i + 1 < marker_pose_options.fixed_marker_ids.size()) {
        marker_ids_ss << ",";
      }
    }
    PRINT_DEBUG("  - marker fixed ids: %s\n", marker_ids_ss.str().c_str());
    PRINT_DEBUG("  - marker time tolerance: %.3f\n", marker_pose_options.time_tolerance_s);
    PRINT_DEBUG("  - marker chi2 gate: %.3f\n", marker_pose_options.chi2_gate);
    PRINT_DEBUG("  - marker noise multiplier: %.3f\n", marker_pose_options.noise_multiplier);
    PRINT_DEBUG("  - marker update max dp/dtheta: %.3f m / %.2f deg\n", marker_pose_options.max_update_translation_m,
                marker_pose_options.max_update_rotation_deg);
    PRINT_DEBUG("  - marker reset dp/dtheta: %.3f m / %.2f deg\n", marker_pose_options.reset_translation_m,
                marker_pose_options.reset_rotation_deg);
