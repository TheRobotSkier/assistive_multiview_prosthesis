...
protected:
  ...
  /// Add a marker measurement to the short reset velocity-fit history
  void record_marker_measurement(const MarkerPoseMeasurement &measurement);

  /// Estimate marker_map velocity from recent accepted marker observations
  bool marker_velocity_fit(Eigen::Vector3d &velocity, Eigen::Matrix3d &velocity_covariance) const;

  /// Test if an innovation result should use the explicit marker reset path
  bool should_marker_reset(const MarkerPoseMeasurement &measurement, const MarkerPoseUpdateResult &innovation) const;

  /// Reset current OpenVINS gauge/state into marker_map using pose and fitted velocity
