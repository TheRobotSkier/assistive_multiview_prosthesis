...
fn main() {
    ...
    let grasp_types = [
        (1i32, "cylindrical", score_cylindrical as fn(&FingerLUT, &grasp_preshaping::pointcloud_helper::Tsdf, &Matrix4<f64>, f32) -> grasp_preshaping::planner::GraspScoreResult),
        (2i32, "pinch", score_pinch as fn(&FingerLUT, &grasp_preshaping::pointcloud_helper::Tsdf, &Matrix4<f64>, f32) -> grasp_preshaping::planner::GraspScoreResult),
        (3i32, "lateral", score_lateral as fn(&FingerLUT, &grasp_preshaping::pointcloud_helper::Tsdf, &Matrix4<f64>, f32) -> grasp_preshaping::planner::GraspScoreResult),
    ];
    ...
    for (si, sp) in samples.iter().enumerate() {
        ...
        for &(gt_int, _gt_name, scorer) in &grasp_types {
