...
Resolve the 100% pointcloud drop rate by (1) fixing confirmed bugs in the Jetson relay gating logic (Issues A & D), (2) confirming the `slop` parameter situation (Issue C), (3) replacing the failing `depth_image_proc::PointCloudXyzrgbNode` with a custom NumPy-based pointcloud assembler that bypasses all multi-topic time synchronization, and (4) restamping image/depth timestamps to system clock on the Jetson relay so downstream TF lookups are temporally accurate.
...
**Fix:** Swap the gate order — check the token gate FIRST, then the rate gate. This ensures only token-matching frames consume rate budget.

---

### Issue B: Incompatible Time Domains — CONFIRMED (addressed by restamping)

**Finding:** The report shows `peak |clock_off| = 5.270s`, but the sysmon section clarifies: `true chrony drift is only 0.1ms`. The 5.27s offset is NOT clock skew between Jetson and host — it is the difference between the RealSense ASIC hardware clock (used for image/depth timestamps) and the system clock (used for camera_info and odom).

**Impact on TF:** The fusion node (`pointcloud_fusion_node.py:451-467`) detects clock skew > 1.0s and falls back to `Time(0)` (latest available transform) instead of the temporally-correct one. For a moving robot, this means the pointcloud is transformed using a transform that may be tens of milliseconds too recent, introducing positional error proportional to velocity × delay.

**Resolution:** Restamp depth and image headers with Jetson system time (`get_clock().now()`) in the relay, right before publishing. Since Jetson↔host chrony drift is 0.1ms, this puts all messages in a unified clock domain that matches TF transforms (which come from OpenVINS using system clock). This is addressed in Phase 1.4 below.

---

### Issue C: Slop Parameter — CONFIRMED: NON-FUNCTIONAL
...
**Fix:** Change the `relay_img_hz` launch argument default from `"5.0"` to `""` (empty string), so the master override flows through correctly.

---

## TF Timestamp Accuracy Analysis

### The Problem

ROS2 TF2 stores transforms indexed by timestamp. When the fusion node calls `lookup_transform_full(target, stamp, source, stamp, ...)` at `pointcloud_fusion_node.py:525-530`, it searches for a transform at the exact time `stamp`. If `stamp` is in ASIC clock domain (5.27s offset from system clock):

1. The fusion node's `_stamp_from_msg` method (line 451-467) detects the skew and sets `_clock_skew_detected = True`
2. All subsequent TF lookups fall back to `lookup_transform(target, source, Time(0))` — the **latest** available transform
3. For a robot moving at 0.5 m/s, a 100ms processing delay introduces 5cm of positional error in the pointcloud

### The Solution: System-Time Restamping

By restamping depth and image headers with `self.get_clock().now()` on the Jetson relay (which is chrony-synced to host at 0.1ms drift), the pointcloud header stamp will be in the same clock domain as TF transforms. This means:

- The fusion node's `_use_header_stamp_age` path works correctly — it finds the transform at the actual capture time
- TF2's `lookup_transform_full` with a small tolerance (`transform_tolerance`) can look slightly into the future if the transform hasn't arrived yet
- The pointcloud is spatially accurate relative to the robot's pose at capture time

### Why This Works with the Custom Assembler

The custom assembler uses the depth frame's header stamp for the output pointcloud. With restamping:
- Depth timestamp = system time at relay moment (capture + ~5ms relay latency)
- Pointcloud timestamp = depth timestamp
- TF transform at that timestamp = robot pose at capture time (within 5ms)
- Result: pointcloud is correctly positioned in the world frame

---

## Implementation Plan

### Phase 1: Fix Jetson Relay Bugs (Issues A, D, + Timestamp Restamping)

- [x] **1.1. Fix cascaded gate order in `_on_depth` (Issue A)**
  ...
  - Current order (lines 731-734):
    ```
    if not self._gates[f"depth_{camera}"].should_publish():  # consumes slot
        return
    if not self._stamp_matches_token(msg.header.stamp, camera):  # may reject
        return
    ```
  - New order:
    ```
    if not self._stamp_matches_token(msg.header.stamp, camera):  # filter first
        return
    if not self._gates[f"depth_{camera}"].should_publish():  # then throttle
        return
    ```

- [x] **1.2. Fix `relay_img_hz` default value (Issue D)**
...
- [x] **1.3. Restamp depth and image headers with system time (Issue B / TF accuracy)**
  - File: `multiview_prosthesis-jetson_docker/docker_ws/multi_cam_localization/sensor_fusion_bringup/scripts/jetson_relay.py`
  - In `_on_img` (line 706-717): after the token gate passes and before publishing, restamp the header:
    - `msg.header.stamp = self.get_clock().now().to_msg()`
  - In `_on_depth` (line 719-743): after both gates pass and before publishing, restamp the header:
    - `msg.header.stamp = self.get_clock().now().to_msg()`
  - In `_on_ci` (line 752-772): restamp camera_info header similarly (camera_info already uses system time on the RealSense driver side, but restamping ensures consistency).
  - **Critical ordering note:** The token gate in `_on_depth` uses `_stamp_matches_token()` which compares the depth's ASIC stamp to the image's ASIC stamp. This comparison must happen BEFORE restamping. The restamping must occur AFTER the token check passes, just before `publish()`.
  - Rationale: The Jetson system clock is chrony-synced to the host at 0.1ms drift. Restamping at the relay puts all messages in a unified clock domain that matches TF transforms from OpenVINS (which uses system clock). This ensures the fusion node's `lookup_transform_full` finds the temporally-correct transform.
  - Update the comments on lines 714, 737-740 to reflect the new restamping behavior.

- [x] **1.4. Remove dead code on line 91 of dynamic_id2_arm_update_live.launch.py**
...
- [x] **2.1. Create `naive_pointcloud_assembler.py` in the camera package**
...
- [x] **2.2. Implement depth-to-XYZ deprojection using NumPy**
...
- [x] **2.3. Implement RGB color assignment**
...
- [x] **2.4. Build and publish PointCloud2 message**
  ...
  - **Set `header.stamp` to the depth image's header stamp** (which is now in system clock domain after Phase 1.3 restamping). This is critical for TF accuracy — the fusion node will use this stamp for `lookup_transform_full` to find the temporally-correct transform.
  - Publish to `/jetson/{side}/points` (same topic the fusion node subscribes to).

- [x] **2.5. Register the node in setup.py**
  - File: `src/camera/setup.py`
  - Add entry point: `"naive_pointcloud_assembler = camera.naive_pointcloud_assembler:main"`

- [x] **2.6. Replace depth_image_proc nodes in pipeline.launch.py**
...
- [x] **3.3. Verify end-to-end with `make run-camera-log-debug`**
  ...
  - Verify `clock_off` values in the analysis report are now < 50ms (confirming restamping worked).
  - Verify the fusion node does NOT log the "Clock skew exceeds threshold" warning (confirming timestamps are in the correct domain).
...
- [ ] `clock_off` values in analysis report < 50ms (restamping successful)
- [ ] Fusion node does NOT log "Clock skew exceeds threshold" warning (timestamps in correct domain)
- [ ] TF jump count in analysis report is reduced (temporally-accurate transforms)
...
1. **NumPy performance on large depth images**
   ...
   - Mitigation: The depth is already downscaled 2x on the Jetson (640x480 to 320x240). At 320x240, NumPy vectorized operations complete in < 10ms on modern hardware. If needed, add spatial decimation (skip every Nth pixel).
...
4. **Restamping introduces small latency offset**
   - Risk: Restamping with `get_clock().now()` at relay time adds the relay processing latency (~5ms) to the timestamp. This means the pointcloud timestamp is slightly after the actual capture.
   - Mitigation: This is acceptable — the fusion node's `transform_tolerance` parameter (default 0.1s) covers this gap. The alternative (ASIC timestamps) is far worse (5.27s offset). For applications requiring sub-ms accuracy, a hardware-sync solution would be needed, but that is out of scope.

5. **Jetson-side changes require Docker rebuild**
...
6. **Token gate still uses ASIC timestamps after restamping**
   - Risk: If restamping happens before the token check, the token comparison breaks.
   - Mitigation: Restamping must occur AFTER the token check passes, just before `publish()`. The token gate compares ASIC timestamps between depth and image — this comparison is unaffected by restamping because both depth and image retain their ASIC stamps until after the gate.
...
1. **Fix depth_image_proc synchronization instead of replacing it**
   ...
   - Trade-off: Still depends on the fragile 4-way ApproximateTimeSynchronizer. Even with restamping, the synchronizer adds latency and complexity. The custom assembler is simpler and more robust.
...
2. **Use ExactTime synchronizer with restamped messages**
   ...
   - Trade-off: Loses the original capture timestamp granularity. The custom assembler approach is strictly better because it doesn't need synchronization at all.
...
4. **Restamp at the decompress bridge instead of the Jetson relay**
   - Trade-off: Adds host-side complexity and introduces network latency into the timestamp. Restamping on the Jetson (chrony-synced at 0.1ms) is more accurate because the timestamp is set closer to the capture moment.
