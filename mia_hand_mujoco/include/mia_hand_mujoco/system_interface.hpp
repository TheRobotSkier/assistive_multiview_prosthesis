#ifndef MIA_HAND_MUJOCO_SYSTEM_INTERFACE_HPP
#define MIA_HAND_MUJOCO_SYSTEM_INTERFACE_HPP
...
public:
  ...
  /**
   * \brief Function called at Mia Hand system interface cleanup.
   */
...
private:
  ...
  /**
   * \brief Function executed in #sim_trd_ background thread.
   */
  void sim_trd_fun();

  std::unique_ptr<rclcpp::Logger> logger_;  //!< For info or error messages.

  std::string xml_model_path_;   //!< Path to Mia Hand XML model.
  ...
  std::thread sim_trd_;  //!< Simulation thread.
};
}  // namespace

#endif  // MIA_HAND_MUJOCO_SYSTEM_INTERFACE_HPP
