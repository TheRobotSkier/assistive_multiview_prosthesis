...
#
# This script configures the Linux kernel on the HOST machine to handle
# high-throughput fragmented UDP traffic from the Jetson (two RealSense
# PointCloud2 streams at 15-30 Hz, each 5-8 MB, arriving as ~120 fragments).
#
...
#   The host receives two PointCloud2 streams (~5-8 MB each), and each incoming
#   UDP fragment must be buffered in the socket while IP reassembly runs.
#   Default ~208 KB is far too small — a single PartCloud2 needs ~8 MB of buffer.
declare -r RMEM_MAX=2147483647

# net.core.wmem_max: maximum socket send buffer size (bytes).
#   2147483647 = 2 GB.  Symmetrical ceiling for the send side.  The host publishes
#   /fused_pointcloud (8-15 MB) which may be subscribed by Jetson-side tools.
...
#
# Two RealSense D435i PointCloud2 streams arrive from the Jetson over a
# direct Ethernet link.  Each cloud is 5-8 MB, fragmented by CycloneDDS
# into ~120 RTPS messages of 64 KB each.  Without these settings the
# default Linux kernel buffers (~208 KB rmem_max, ~4 MB ipfrag) drop
# fragments under load, causing ros2 topic hz/echo to crash.
...
echo ""
echo "REMINDER: The Jetson also needs these settings."
echo "  Run from host:  make network-tune-jetson"
echo "  Or run both:    make network-tune-all"
