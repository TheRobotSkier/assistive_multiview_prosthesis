#!/bin/bash
# check_final_jetson_topics.sh — verify final dual-D435i Jetson topics.
...
check_topic_once() {
    ...
    local type="$2"
    echo "Checking ${topic}"
    timeout "$TIMEOUT" ros2 topic echo --once "$topic" "$type" >/tmp/check_final_topic.log
}
...
echo "Final dual-D435i Jetson camera/IMU topics OK"
