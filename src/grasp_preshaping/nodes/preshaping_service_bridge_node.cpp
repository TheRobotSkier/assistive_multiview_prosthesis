...
public:
    ...
    publish_initial_commands_(declare_parameter<bool>("publish_initial_commands", true))
  {
    ...
    const std::string hand_pose_topic =
      declare_parameter<std::string>("hand_pose_topic", "/hand_pose");
    ...
    const std::string index_cmd_topic =
      declare_parameter<std::string>("index_cmd_topic", "/index_pos_ff_controller/commands");
    const std::string mrl_cmd_topic =
      declare_parameter<std::string>("mrl_cmd_topic", "/mrl_pos_ff_controller/commands");
    const std::string wrist_pose_topic =
    ...
    const std::string grasp_type_topic =
      declare_parameter<std::string>("grasp_type_topic", "/grasp_preshaping/grasp_type");
    ...
    target_finger_closures_pub_ = create_publisher<std_msgs::msg::Float64MultiArray>(
      target_closures_topic, rclcpp::QoS(10).transient_local());
    ...
    RCLCPP_INFO(
      get_logger(),
      "Preshaping service bridge ready on %s", compute_service.c_str());
  }
...
private:
  void initialize_rust_backend()
  {
    ...
    candidates.emplace_back("/prosthesis_ws/install/grasp_preshaping/lib/libgrasp_preshaping.so");
    candidates.emplace_back("/prosthesis_ws/install/lib/libgrasp_preshaping.so");
    candidates.emplace_back("/prosthesis_ws/src/grasp_preshaping/lib/libgrasp_preshaping.so");
    candidates.emplace_back("/prosthesis_ws/src/grasp_preshaping/target/release/libgrasp_preshaping.so");
