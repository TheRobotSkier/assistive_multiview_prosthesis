...
def generate_launch_description():
    ...
    # Single-machine merge: remap the native OpenVINS odom topic to the
    # /jetson/arm/odom name the host pipeline expects (see merge plan §5.4).
    # This is a free DDS alias — no relay node, no extra compute.
    openvins_phase2 = Node(
        ...
        output="screen",
        remappings=[
            ("/ov_msckf_arm/odomimu", LaunchConfiguration("odom_output_topic")),
        ],
        parameters=[
            {"use_sim_time": LaunchConfiguration("use_sim_time")},
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            description="Output topic for OpenVINS odometry. Defaults to the "
                        "/jetson/arm/odom name the host pipeline expects.",
        ),
        DeclareLaunchArgument(
            "start_camera",
            default_value="true",
        ...
        DeclareLaunchArgument("dynamic_arm_reanchor_trigger_translation_m", default_value="0.35"),
        DeclareLaunchArgument("dynamic_arm_reanchor_trigger_rotation_deg", default_value="15.0"),
