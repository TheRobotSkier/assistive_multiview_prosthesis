...
def generate_launch_description():
    ...
    # /jetson/head/odom name the host pipeline expects (see merge plan §5.4).
    ...
    openvins_phase2 = Node(
        ...
        output="screen",
        remappings=[
            ("/ov_msckf/odomimu", LaunchConfiguration("odom_output_topic")),
        ],
        parameters=[
            {"use_sim_time": LaunchConfiguration("use_sim_time")},
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            description="Output topic for OpenVINS odometry. Defaults to the "
                        "/jetson/head/odom name the host pipeline expects.",
        ),
        DeclareLaunchArgument(
            "start_camera",
            default_value="true",
