"""Launch a single D435i camera from the sensor_fusion_bringup config.

Uses the official realsense2_camera rs_launch.py and applies the Jetson
NEON pointcloud fix via a TimerAction after camera initialization.

Launch arguments:
    camera_key            Which camera from config/d435i_cameras.yaml (default: head)
...
def _setup_launch(context, *args, **kwargs):
    ...
    camera_key = LaunchConfiguration("camera_key").perform(context)

    if camera_key not in data["cameras"]:
        raise RuntimeError(
            f"Unknown camera_key='{camera_key}'. Expected one of: {list(data['cameras'].keys())}"
        )
