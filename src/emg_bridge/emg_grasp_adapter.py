    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            confidence_threshold: minimum confidence for any non-neutral gesture.
            stale_timeout_s: max age of last message before signal goes neutral.
            ...
            node: optional ROS 2 node to auto-create subscriptions.
        """
        self._aliases: Dict[str, str] = dict(aliases) if aliases else {}
        self._confidence_threshold = float(confidence_threshold)
        self._stale_timeout_s = float(stale_timeout_s)
        self._open_min_control = float(open_min_control)
        self._open_min_hold_s = float(open_min_hold_s)
        ...
        self._current_signal = EmgSignal()

        # ROS subscriptions
        self._subs: list = []
        if HAS_ROS and node is not None:
            self._attach_to_node(node)

    # ── ROS plumbing ─────────────────────────────────────────────────────────

    def _attach_to_node(self, node: Node) -> None:
        qos = QoSProfile(depth=1, durability=DurabilityPolicy.TRANSIENT_LOCAL)
        self._subs.append(
            node.create_subscription(String, "/emg/gesture_name", self._on_gesture_name, qos)
        )
        self._subs.append(
            node.create_subscription(Float32, "/emg/confidence", self._on_confidence, qos)
        )
        self._subs.append(
            node.create_subscription(Float32, "/emg/proportional", self._on_proportional, qos)
        )
