...
Parameters:
    ...
    open_hold_duration_s  (double)– seconds OPEN must be held (default 0.5)
    open_prop_threshold   (double)– proportional threshold for OPEN safety (default 0.7)
    power_hold_duration_s (double)– seconds POWER must be held (default 0.5)
...
class EmgGraspControllerNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("open_hold_duration_s", 0.5)
        self.declare_parameter("open_prop_threshold", 0.7)
        self.declare_parameter("power_hold_duration_s", 0.5)
    ...
    def _tick(self) -> None:
        ...
        if self._data_received and (now - self._last_msg_time) > self._stale_timeout:
            inp = EmgInput(
                gesture=self._sm._cfg.gesture_rest,
                confidence=0.0,
                proportional=0.0,
                timestamp=now,
            )
